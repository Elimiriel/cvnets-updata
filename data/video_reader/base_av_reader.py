...
class BaseAVReader(object):
    ...
    def read_video_file_into_clips(
        ...
        **kwargs,
        ...
        Returns:
            A dictionary with video/audio as tensor and metadata. The format of
            the metadata is the following:
            {
                "video_fps": float,
                "audio_fps": int,
                "video_frame_timestamps": [num_clips x video_frames_per_clip] tensor.
            }
        """
        ...
        av_data = self.read_video(
            ...
            custom_frame_processor=custom_frame_transforms,
        )
        ...
        if torch_audio is None:
            ...
            audio_sec = num_audio_frames / audio_fps
            video_sec = num_video_frames / original_video_fps
            # Making sure audio and video are of same length.
            if numpy.fabs(audio_sec - video_sec) >= 0.1:
                raise VideoDurationDoesNotMatchAudioDurationError(
                    f"Audio duration {audio_sec} video duration {video_sec} might be different."
                )
