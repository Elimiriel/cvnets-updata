...
class BaseImageClassificationDataset(BaseImageDataset, ImageFolder):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for collate_fn in [
            ...
            if not hasattr(opts, collate_fn):
                setattr(opts, collate_fn, "image_classification_data_collate_fn")
    ...
    def add_arguments(cls, parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
        ...
        group.add_argument(
            ...
            action="store_true",
            default=False,
            help="Enabling this flag allows us to use sample efficient training. "
            "Currently only tested on classification tasks.",
        )
        group.add_argument(
            "--dataset.sample-efficient-training.sample-confidence",
            type=float,
            default=0.5,
            help="Confidence for identifying samples as easy or hard. If the "
            "predicted classification probability of a sample is greater than "
            "this value, then sample is classified as easy. Otherwise, it is "
            "classified as hard. Defaults to 0.5.",
        )
        group.add_argument(
            "--dataset.sample-efficient-training.find-easy-samples-every-k-epochs",
            type=int,
            default=5,
            help="Find easy samples after every K epochs. Defaults to 5.",
        )
        group.add_argument(
    ...
    def share_dataset_arguments(self) -> Dict[str, Any]:
        ...
        share_dataset_specific_opts["model.classification.n_classes"] = self.n_classes
