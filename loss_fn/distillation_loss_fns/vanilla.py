...
class VanillaDistillationLoss(BaseCriteria):
    ...
    def __init__(self, opts):
        ...
        adaptive_wt_balance = getattr(
            opts, "loss.distillation.vanilla_adaptive_weight_balance", False
        )
        ...
        distill_type = getattr(
            opts, "loss.distillation.vanilla_distillation_type", "soft"
        )
        super().__init__()
    ...
    def build_teacher_model(opts) -> nn.Module:
        teacher_model_name = getattr(
            opts, "loss.distillation.vanilla_teacher_model", "resnet_50"
        ).lower()
        ...
        pretrained_wts_path = getattr(
            opts, "loss.distillation.vanilla_teacher_model_weights", None
        )
        ...
        if pretrained_wts_path is not None and os.path.isfile(pretrained_wts_path):
            ...
            raise RuntimeError(
                "Pretrained weights are required for teacher model ({}) in the distillation loss".format(
                    teacher_model_name
                )
            )
    ...
    def build_label_loss_fn(opts) -> BaseCriteria:
        label_loss_name = getattr(
            opts, "loss.distillation.vanilla_label_loss", "cross_entropy"
        )
    ...
    def forward(
        self, input_sample: Tensor, prediction: Tensor, target: Tensor
    ) -> Tensor:
        ...
        if self.adaptive and self.step_counter < self.steps:
            self.loss_acc_dist += tensor_to_python_float(
                distillation_loss, is_distributed=self.is_distributed
            )
            self.loss_acc_label += tensor_to_python_float(
                label_loss, is_distributed=self.is_distributed
            )
    ...
    def add_arguments(cls, parser: argparse.ArgumentParser):
        ...
        group.add_argument(
            "--loss.distillation.vanilla-teacher-model",
            type=str,
            default="resnet_50",
            help="Teacher model for distillation",
        )
        group.add_argument(
            "--loss.distillation.vanilla-label-loss",
            type=str,
            default="cross_entropy",
            help="Label loss",
        )
        group.add_argument(
            "--loss.distillation.vanilla-alpha",
            type=float,
            default=0.5,
            help="Contribution of label loss",
        )
        group.add_argument(
            "--loss.distillation.vanilla-tau", type=float, default=1.0, help="Tau value"
        )
        group.add_argument(
            "--loss.distillation.vanilla-adaptive-weight-balance",
            action="store_true",
            help="Adaptively balance the contribution of label and distribution loss",
        )
        group.add_argument(
            "--loss.distillation.vanilla-accum-iterations",
            type=int,
            default=10000,
            help="Number of iterations for accumulating loss",
        )
        group.add_argument(
            "--loss.distillation.vanilla-weight-update-freq",
            type=int,
            default=100,
            help="Update freq for updating the weights for label and distribution loss",
        )
        group.add_argument(
            "--loss.distillation.vanilla-teacher-model-weights",
            type=str,
            help="Teacher model path",
        )
        group.add_argument(
            "--loss.distillation.vanilla-distillation-type",
            type=str,
            default="soft",
    ...
    def __repr__(self):
        label_loss_str = (
            str(self.label_loss.__repr__()).replace("\n", "").replace("\t", "")
        )
        return "{}(\n\t label_loss_fn={} \n\t teacher_model={} \n\t alpha={} \n\t tau={} \n\t adaptive={} \n\t accum_iter={} \n\t update_freq={} \n)".format(
            self.__class__.__name__,
            label_loss_str,
